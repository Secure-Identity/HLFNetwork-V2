...
{
   "peer_name": "peer0.org1.secretidentity.com",
   "channelid": "mychannel",
   "use_couchdb":false,
...
async function main() {
    try {
        ...
        const userExists = await wallet.get('prerit1');
        if (!userExists) {
            console.log('An identity for the user "prerit1" does not exist in the wallet');
        ...
        await gateway.connect(ccp, { wallet, identity: 'prerit1', discovery: { enabled: true, asLocalhost: true } });
