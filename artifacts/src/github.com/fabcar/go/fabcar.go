...
func (s *SmartContract) keymaker(APIstub shim.ChaincodeStubInterface) sc.Response {
	...
	// randomKey := rand.Int63() % order
	randomKey := rand.Int63n(order * 10)
	//fmt.Printf("Random Key: %d\n", randomKey)
	poly := genEquation(randomKey, 2)
	//fmt.Printf("length of poly: %d\n", len(poly))
	//fmt.Println(poly)
	shardX, shardY := genShards(poly)
	//fmt.Println("y = ", poly[0], "x^2 + ", poly[1], "x + ", poly[2])
	// for i := range shardX {
	// 	fmt.Printf("shard[%d]=(%.1f, %.1f), evaluated=(%.1f, %.1f)\n", i, shardX[i], shardY[i], shardX[i], evalPoly(poly, shardX[i]))
	// }

	xshard:=strings.Trim(strings.Join(strings.Fields(fmt.Sprint(shardX)),","), "[]") + "\n"

	yshard:=strings.Trim(strings.Join(strings.Fields(fmt.Sprint(shardY)), ","), "[]")
	...
	_, err = f.WriteString(xshard)
	if err != nil {
		return shim.Error(err.Error())
	}
	_, err = f.WriteString(yshard)
	if err != nil {
		return shim.Error(err.Error())
	}
	result := xshard + yshard
